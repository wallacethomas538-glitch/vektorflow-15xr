...
# from payment import check_subscription_status   ← REMOVED THIS LINE
...
async def agent_command(command: AgentCommand):
    ...
    FIXED: Properly passes email, agent_name, command, and params to run_agent_task.
    """
    try:
        # Set default email for commander
        email = "commander@vektorflow.com"
        
        # Get user's LLM keys
        user_keys = get_llm_keys(email)
        
        # Detect intent using vektor_agent.py
        intent = detect_intent(command.command, {})
        
        # FIXED: Call run_agent_task with correct number of arguments
        # Signature: run_agent_task(email, agent_name, command, params)
