...
Commander-Only Test Login + Full OAuth for Customers
"""
...
# ========== CJ API KEY (from environment variables) ==========
...
# ========== COMMANDER-ONLY TEST LOGIN ==========
@app.get("/commander-login")
async def commander_login(email: str = None, password: str = None):
    """
    COMMANDER-ONLY LOGIN
    Only the Commander can use this. Access is hardcoded.
    URL: /commander-login?email=commander@vektorflow.com&password=test123
    """
    # Verify credentials
    if email != COMMANDER_EMAIL or password != COMMANDER_PASSWORD:
        return {"error": "Access denied. Commander only."}
    
    # Check if test user exists
    user = get_user(COMMANDER_EMAIL)
    if not user:
        # Create test user if doesn't exist
        password_hash = hash_password(COMMANDER_PASSWORD)
        create_user(COMMANDER_EMAIL, password_hash, "Commander Store")
        create_subscription(COMMANDER_EMAIL, "free")
    
    # Generate JWT token
    token = create_jwt(COMMANDER_EMAIL)
    
    # Return token and auto-login redirect
    return {
        "status": "success",
        "message": "Commander login successful.",
        "access_token": token,
        "email": COMMANDER_EMAIL,
        "redirect_url": f"/?token={token}",
        "instructions": "Copy the access_token and add it to the URL: /?token=YOUR_TOKEN"
    }

@app.get("/commander-dashboard")
async def commander_dashboard():
    """
    Direct login for Commander - auto redirects to dashboard.
    Just go to: /commander-dashboard
    """
...
@app.get("/commander/control")
async def commander_control(email: str = Depends(get_current_user)):
    """
    Commander Control Panel - Full system overview.
    Only accessible with a valid token.
    """
    # Verify this is the Commander
    if email != COMMANDER_EMAIL:
        raise HTTPException(403, "Access denied. Commander only.")
    
    conn = get_db()
    cursor = conn.cursor()
    
    # Get all users
    cursor.execute("SELECT email, store_name, tier, trial_expires, created_at FROM users")
    users = cursor.fetchall()
    
    # Get all stores
    cursor.execute("SELECT * FROM user_stores")
    stores = cursor.fetchall()
    
    # Get all subscriptions
    cursor.execute("SELECT email, plan, status, current_period_end FROM subscriptions")
    subscriptions = cursor.fetchall()
    
    # Get task history
    cursor.execute("SELECT email, agent_name, task, status, created_at FROM task_history ORDER BY created_at DESC LIMIT 20")
    tasks = cursor.fetchall()
    
    # Get memory count
    cursor.execute("SELECT email, COUNT(*) as count FROM vektor_memory GROUP BY email")
    memory_counts = cursor.fetchall()
    
    conn.close()
    
    return {
        "commander": COMMANDER_EMAIL,
        ...
        "memory_counts": [dict(m) for m in memory_counts]
    }
...
async def cj_search(keyword: str, email: str = Depends(get_current_user)):
    products = await search_cj_products(keyword, CJ_API_KEY)
