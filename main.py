...
OAuth-first, 7-day trial, subscription enforcement, GTM Toolkit
"""
...
from database import check_and_block_access, extend_trial
init_db()
...
def save_icp(data: ICPData, email: str = Depends(get_current_user)):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        INSERT OR REPLACE INTO users (email, icp_data, updated_at)
        VALUES (?, ?, CURRENT_TIMESTAMP)
    """, (email, json.dumps(data.dict())))
    conn.commit()
    conn.close()
    return {"status": "saved"}
...
def get_icp(email: str = Depends(get_current_user)):
    ...
    row = cursor.fetchone()
    conn.close()
    return {"icp": json.loads(row[0]) if row and row[0] else None}

# ========== OAUTH INITIATE ==========
@app.post("/store/initiate")
async def initiate_store_connection(request: StoreInitiateRequest):
    """Step 1: User provides store URL, we initiate OAuth"""
...
async def store_callback(code: str, shop: str, state: str, platform: str):
    """Step 2: OAuth callback - creates account and starts trial"""
    conn = get_db()
    cursor = conn.cursor()
...
async def generate_outreach_sequence(req: OutreachRequest, email: str = Depends(get_current_user)):
    ...
    Make each email personal, concise, and value-focused.
    """
