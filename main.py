...
Complete with Groq Models Endpoint, CJ Dropshipping, Inventory
"""
...
async def get_groq_models(email: str = Depends(get_current_user)):
    ...
    if not groq_key:
        return {"success": False, "error": "No Groq API key found", "models": [
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code != 200:
                return {"success": False, "error": f"Groq API error: {response.status_code}", "models": [
                    {"id": "llama-3.3-70b-versatile", "name": "Llama 3.3 70B"},
                    {"id": "openai/gpt-oss-120b", "name": "GPT OSS 120B"}
                ]}
            ...
            for model in all_models:
                ...
                if any(x in model_id for x in ['llama', 'gpt-oss', 'qwen', 'mixtral']):
                    if "instruct" in model_id or "versatile" in model_id or "oss" in model_id:
                        active_models.append({
                            "id": model_id,
                            "name": model_id.replace("-", " ").title(),
                            "created": model.get("created")
                        })
            active_models.sort(key=lambda x: x.get('created', 0), reverse=True)
...
def add_inventory_item(item: InventoryItem, email: str = Depends(get_current_user)):
    ...
    conn.commit()
    conn.close()
    return {"status": "added", "item": item.sku}
...
async def search_cj_products(keyword: str, email: str = Depends(get_current_user)):
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.get(f"{CJ_MCP_URL}/search", params={"keyword": keyword})
            if response.status_code == 200:
...
async def call_llm(request: LLMRequest, email: str = Depends(get_current_user)):
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("SELECT groq_key, gemini_key, hf_key FROM user_keys WHERE email = ?", (email,))
    row = cursor.fetchone()
    conn.close()
    
    user_keys = dict(row) if row else {}
