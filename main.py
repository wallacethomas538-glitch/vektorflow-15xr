...
Phase 1: Langfuse Observability + Coqui TTS ready
"""
...
async def call_llm(prompt: str, model: str, user_keys: Dict) -> Dict:
    ...
    if LANGFUSE_ENABLED:
        try:
            langfuse = Langfuse(
                ...
                host=os.environ.get("LANGFUSE_HOST", "https://langfuse.onrender.com")
            )
            trace = langfuse.trace(name="llm_call", metadata={"model": model, "provider": provider})
            span = trace.span(name="provider_call")
        except:
            pass
    ...
    try:
        if provider == "groq":
            ...
            result = await call_deepseek(prompt, api_key, model)
        elif provider == "gemini":
            result = await call_gemini(prompt, api_key, model)
        elif provider == "huggingface":
            result = await call_huggingface(prompt, api_key, model)
        elif provider == "openai":
            result = await call_openai(prompt, api_key, model)
        elif provider == "anthropic":
            result = await call_anthropic(prompt, api_key, model)
        elif provider == "mistral":
            result = await call_mistral(prompt, api_key, model)
        elif provider == "cohere":
            result = await call_cohere(prompt, api_key, model)
        else:
            result = {"success": False, "error": f"Provider {provider} not configured"}
