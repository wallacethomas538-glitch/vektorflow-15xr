...
# ========== ADMIN: SET GROQ KEY DIRECTLY ==========
@app.get("/admin/set-groq-key")
def set_groq_key(email: str = "commander@vektorflow.com", key: str = ""):
    if not key:
        return {"status": "error", "message": "No key provided. Use ?key=YOUR_GROQ_KEY"}
    
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("""
        INSERT OR REPLACE INTO user_keys (email, groq_key, updated_at)
        VALUES (?, ?, CURRENT_TIMESTAMP)
    """, (email, key))
    conn.commit()
    conn.close()
    return {"status": "saved", "email": email, "message": f"Groq key saved for {email}"}
...
async def call_llm(request: LLMRequest, email: str = Depends(get_current_user)):
    ...
    if not groq_key:
        return {"success": False, "error": "No Groq API key found. Use /admin/set-groq-key?key=YOUR_KEY to add it."}
