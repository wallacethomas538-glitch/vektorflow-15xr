...
With EcomLib, stockpyl, CJ Dropshipping MCP, and Debug Endpoint
"""
...
def check_env():
    ...
    return {
        ...
        "message": "If groq_key_exists is false, add GROQ_API_KEY to Render environment variables"
    }
    ...
    if STOCKPYL_AVAILABLE:
        try:
            if item.unit_cost > 0 and item.selling_price > item.unit_cost:
                reorder_suggestion = int((2 * 1000 * item.unit_cost) ** 0.5)
        except:
            pass
...
# ========== CJ DROPSHIPPING MCP INTEGRATION ==========
...
async def call_llm(request: LLMRequest, email: str = Depends(get_current_user)):
    # First try: Get key from user's saved keys in database
    conn = get_db()
    cursor = conn.cursor()
    ...
    # Second try: Fallback to environment variable
    if not groq_key:
        groq_key = os.environ.get("GROQ_API_KEY")
    
    # Third try: Still no key? Return error
    if not groq_key:
        return {
            "success": False, 
            "error": "No Groq API key found. Add your key in the Dashboard (Your API Keys section) or set GROQ_API_KEY in Render environment variables.",
            "debug_hint": "Visit /debug/check-env to see if GROQ_API_KEY is set in environment"
        }
    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if response.status_code == 200:
                ...
                return {"success": False, "error": f"Groq API error: {response.status_code} - {error_detail}"}
    except Exception as e:
